class ConvertRomanNumeral {
  ...
  validate(inputString) {
    const regexp = /(^[IVXLCDM]+$)|(^[0-9]+$)/gi;
    if (regexp.exec(inputString)) {
      return inputString;
    } else {
      return null;
    }
  ...
  process(input) {
    let valid = this.validate(input.trim());
    if (!valid) {
      return "ERROR";
    } else {
      let val = parseInt(valid);
      if (isNaN(val)) {
        return this.fromRomanToArabic(valid.toUpperCase());
      } else {
        return this.fromArabicToRoman(val);
      }
    }
  }
  ...
  /**
     ...
     */
  fromRomanToArabic(romanNumberString) {
    let roman_number = romanNumberString.trim().toUpperCase();
